...
void* do_alloc(size_t size, const string &type) {
	...
	if (mem == NULL) {
		cerr << "Internal error: " << "Cannot allocate space for " << type << endl;
		exit(-1);
	}
...
// algorithms
inline bool trevisan_extract(uint64_t i, vector<uint64_t> &indices, bitfield<unsigned int, uint64_t> &init_rand_bf, unsigned int *y_S_i, params &params, wd_file_lock_type &wd_file_lock, bitext *bext, weakdes *wd, uint64_t t, int *extracted_rand, uint64_t num_rand_bits, unsigned short offset) {
	...
	if (params.save_weakdes) {
		{
			...
			params.wd_out.write(reinterpret_cast<const char*>(&indices[0]), t * sizeof(indices[0]));
		}
	}
	...
	for (uint64_t count = 0; count < t; count++) {
		...
		if (init_rand_bf.get_bit(indices[count])) {
			y_S_i[count / BITS_PER_TYPE(unsigned int)] |= ((unsigned int) 1 << (count % BITS_PER_TYPE(unsigned int)));
		}
	}
...
void trevisan_dispatcher(class weakdes *wd, class bitext *bext, params &params) {
	...
	cout << "1-bit extractor requires " << t << " bits per " << "extracted bit" << endl;
	...
	if (params.verbose) {
		cout.precision(3);
		cout << "Total amount of initial randomness: " << num_rand_bits << " (" << fixed << (((float) num_rand_bits) / (1024 * 1024)) << " MiBit)" << endl;
		cout << "Ratio of extracted bits to initial randomness: " << ((long double) params.pp.m) / num_rand_bits << endl;
		cout << "-------------------------------------------------------------" << endl << endl;
	}
	...
	if (params.save_weakdes) {
		...
		offset = sizeof(unsigned short) + sizeof(wd_type) + 3 * sizeof(uint64_t);
	...
	init_rand_bf.set_raw_data(create_randomness(num_rand_bits, initial_rand_vector), num_rand_bits);
	...
	if (params.bxt == bext_type::RSH_CUDA) {
#ifdef USE_CUDA

		vector<vector<uint64_t> > indices_all;
		vector<vector<unsigned int> > y_S_i_all;

		indices_all.reserve(params.pp.m);
		y_S_i_all.reserve(params.pp.m);

		for (int i=0;i<params.pp.m;i++) {
			vector<uint64_t> indices(t, 0);
			vector<unsigned int> y_S_i(num_rand_bits/BITS_PER_TYPE(unsigned int)+1, 0);
//			y_S_i.reserve(num_rand_bits/BITS_PER_TYPE(unsigned int)+1);
//			indices.reserve(t);

			wd->compute_Si(i, indices);

			memset(y_S_i.data(), 0, num_rand_bits / BITS_PER_BYTE + 1);
			for (uint64_t count = 0; count < t; count++) {
				// Set the bit indexed count in y_S_i to the bit
				// indexed by indices[count] in init_rand.

				if (init_rand_bf.get_bit(indices[count])) {
					y_S_i.at(count / BITS_PER_TYPE(unsigned int)) = y_S_i.at(count / BITS_PER_TYPE(unsigned int)) | ((unsigned int) 1 << (count % BITS_PER_TYPE(unsigned int)));
//					y_S_i[count / BITS_PER_TYPE(unsigned int)] |= ((unsigned int) 1
//							<< (count % BITS_PER_TYPE(unsigned int)));
				}
			}

			indices_all.push_back(indices);
			y_S_i_all.push_back(y_S_i);
		}


		((bitext_rsh_cuda*)bext)->prepare(params.pp.m, y_S_i_all);
//		, params, wd_file_lock, bext, wd, t, num_rand_bits, offset

		for (int i=0;i<params.pp.m;i++) {
//			trevisan_extract(i, indices, init_rand_bf, &y_S_i[0], params, wd_file_lock, bext, wd, t, extracted_rand, num_rand_bits, offset);
			((bitext_rsh_cuda*)bext)->cudaExtract(i, y_S_i_all, extracted_rand);
		}

		((bitext_rsh_cuda*)bext)->followUp();
#endif
	} else {
		...
		// the 1-bit results.
		parallel_for(tbb::blocked_range<uint64_t>(0, params.pp.m), [bext, wd, t, extracted_rand, num_rand_bits, offset,
	...
	cout << "Required " << delta_to_ms(delta) << " ms for " << params.pp.m << " bits extracted from " << params.pp.n << " bits." << endl;

	cout << "Performance: " << params.pp.m / (delta_to_s(delta) * 1000) << " kbits/s" << endl;
	...
	if (params.extRand_file != "") {
		...
		fHandle.write((char*) extracted_rand, (params.pp.m / BITS_PER_TYPE(int) + 1) * sizeof(int));
		fHandle.close();
	}
...
void parse_cmdline(struct params &params, int argc, char **argv) {
	try {
		...
		ValueArg<unsigned long> nArg("n", "inputsize", "Length of input data (bits)", true, 0, "Integer");
		ValueArg<unsigned long> mArg("m", "outputsize", "Length of extracted data (bits)", true, 0, "Integer");
		ValueArg<string> weakdesArg("w", "weakdes", "Weak design construction "
				"(gf2x, gfp, block, aot)", false, "gf2x", "string");
		ValueArg<string> basic_weakdesArg("", "basic-weakdes", "Basic weak design construction for the "
				"block weak design (gf2x, gfp, aot)", false, "gf2x", "string");
		ValueArg<string> bitextArg("x", "bitext", "Bit extractor construction (lu, xor, rsh, rsh_cuda)", false, "xor", "string");
		ValueArg<double> alphaArg("a", "alpha", "Source entropy factor alpha (0 < alpha < 1)", false, 0.9, "Real");
		ValueArg<double> epsArg("e", "eps", "1 bit error probability (0 < eps < 1)", false, 1e-7, "Real");
		ValueArg<double> lu_nuArg("", "lu:NU", "nu parameter for the Lu extractor", false, 0.45, "Integer");
		ValueArg<int> numTasksArg("", "numtasks", "Number of tasks (0 means unlimited)", false, -1, "Integer");
		SwitchArg ignoreEntropyArg("", "ignore-entropy-violation", "Allow minimum entropy requirements", false);
		...
		SwitchArg dryrunArg("", "dry-run", "Only compute parameters, don't extract", false);
		...
		SwitchArg skipBitextArg("", "skip-bitext", "Skip the bit extraction step", false);
		cmd.add(skipBitextArg);

		ValueArg<string> weakdesFileArg("", "weakdes-file", "File to save/load the weak design", false, "", "string");
		cmd.add(weakdesFileArg);

		ValueArg<string> saveExtractedRandomnessArg("", "extRand-file", "File where extracted Randomness is stored ", false, "", "string");
		cmd.add(saveExtractedRandomnessArg);

#ifndef NO_DEBUG
		MultiSwitchArg debugArg("d", "debug", "Emit diagnostic output "
		...
		if (params.wdt == wd_type::BLOCK) {
			...
			if (params.basic_wdt == wd_type::BLOCK) {
				cerr << "Cannot use the block design as basic design for " << "block design" << endl;
				exit(-1);
			}
		}
		...
		if (kiloArg.getValue() && megaArg.getValue()) {
			cout << "Please specify either kilo or mega as multiplier, " << "not both!" << endl;
			exit(-1);
		}
		...
		if (params.pp.lu_nu <= 0 || params.pp.lu_nu >= 0.5) {
			cerr << "nu parameter for the Lu extractor must satisfy " << "0 < nu < 0.5" << endl;
			exit(-1);
		}
		...
		if (params.wdt == wd_type::AOT || (params.wdt == wd_type::BLOCK && params.basic_wdt == wd_type::AOT)) {
			// When the AOT weak design was specified, the filename
			// is not used to save, but to read the data
			if (params.wd_filename == "") {
				cerr << "Cannot use AOT weak design without filename " << "(specify --weakdes=file=<file>)" << endl;
				exit(-1);
			}
			...
			if (!params.wd_out.is_open()) {
				cout << "Error: Cannot open weak design output file!" << endl;
				exit(-1);
			}
		}
	} catch (ArgException &e) {
		cerr << "Error encountered during command line parsing: " << e.error() << " for argument " << e.argId() << endl;
		exit(-1);
	}
}
...
void show_params(struct params &params, bitext *bext, weakdes *wd) {
	...
	if (params.wdt == wd_type::BLOCK) {
		cout << " (basic construction: " << weakdes_to_string(params.basic_wdt) << ", number of blocks: " << dynamic_cast<weakdes_block*>(wd)->get_num_blocks() << ")" << endl;
	}
	cout << endl;

	cout << "Source entropy factor alpha: " << params.pp.alpha << endl;
	cout << "1 bit error eps: " << params.pp.eps << endl;
	cout << "Required source entropy: " << bext->compute_k() << " (available: " << static_cast<uint64_t>(params.pp.alpha * params.pp.n) << ")" << endl;
	if (params.bxt == bext_type::XOR) {
		cout << "XOR-extractor parameter l: " << dynamic_cast<bitext_xor*>(bext)->get_l() << endl;
	}

	if (params.bxt == bext_type::LU) {
		cout << "Lu-extractor parameters: nu=" << params.pp.lu_nu << ", c=" << dynamic_cast<bitext_expander*>(bext)->get_c() << ", l=" << dynamic_cast<bitext_expander*>(bext)->get_l() << ", w=" << dynamic_cast<bitext_expander*>(bext)->get_w() << endl;
	}
...
int dispatch(struct params &params) {
	...
	// if cuda extractor, spawn only one cpu thread
	if (params.bxt == bext_type::RSH_CUDA) {
		params.num_tasks = 1;
	}
...
#error Please use a thread capable openssl
	...
	if (bext->compute_k() > params.pp.alpha * params.pp.n) {
		if (params.ignore_entropy)
			...
			cerr << "Error: ";
		cerr << "Source does not contain sufficient entropy " << "for specified extraction parameters!" << endl;
		if (!params.ignore_entropy) {
			cerr << "(Choose a smaller m, change eps or use a " << "different weak design to satisfy the constraints," << endl << "or specify --ignore-entropy-violation to " << "ignore the constraint)" << endl;
			exit(-1);
		}
	}
